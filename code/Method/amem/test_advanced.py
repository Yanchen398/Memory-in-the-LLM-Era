from IndepthMem.Method.amem.memory_layer import LLMController, AgenticMemorySystem
import os
...
class advancedMemAgent:
    def __init__(self, model, backend, retrieve_k, temperature_c5):
        ...
        self.retriever_llm = LLMController(backend=backend, model=model, api_key="EMPTY")
...
def evaluate_dataset(dataset_path: str, model: str, modelname: str, output_path: Optional[str] = None, ratio: float = 1.0, backend: str = "openai", temperature_c5: float = 0.5, retrieve_k: int = 10):
    ...
    log_filename = f"{modelname}_{backend}_ratio{ratio}_{timestamp}.log"
...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="data/locomodemo.json",
                      help="Path to the dataset file")
    # parser.add_argument("--model", type=str, default="gpt-4o-mini",
    #                   help="OpenAI model to use")
    parser.add_argument("--model", type=str, default="/home/docker/LLaMA-Factory/output/qwen2_5_lora_sft",
                      help="OpenAI model to use")
    parser.add_argument("--modelname", type=str, default="qwen2_5_lora_sft",)
    ...
    evaluate_dataset(dataset_path, args.model, args.modelname, output_path, args.ratio, args.backend, args.temperature_c5, args.retrieve_k)
