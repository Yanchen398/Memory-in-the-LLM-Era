...
class LlmLingua2Segmenter:
    def __init__(self, config: Optional[Dict] = None, shared: bool = False, compressor=None):
        self.config = config
        self.service_url = (
            self.config.get("service_url")
            or os.environ.get("LIGHTMEM_LLMLINGUA_SERVICE_URL")
            or ""
        ).rstrip("/")
        self.service_timeout = float(self.config.get("service_timeout", 300))
    ...
    def propose_cut(self, buffer_texts: List[str]) -> Dict[str, Any]:
        ...
        if n == 0:
            return []
        sequence_length = 2 + sum(
            len(self.tokenizer.encode(text, add_special_tokens=False))
            for text in buffer_texts
        )
        if sequence_length > self.buffer_len:
            return []
        if self.service_url:
            response = requests.post(
                f"{self.service_url}/propose_cut",
                json={"buffer_texts": buffer_texts},
