...
DEFAULT_LOCOMO_RETRIEVE_K = 60
DEFAULT_LONGMEMEVAL_RETRIEVE_K = 20
DEFAULT_RETRIEVE_TOP_KS = [1, 3, 5, 10, 15]
...
def top_k_response_path(output_dir: str, top_k: int) -> str:
    return os.path.join(output_dir, f"top_k_{top_k}", "result_simplified.json")
...
def convert_locomo_longmemeval_sample(sample: Dict, source_record: Optional[Dict] = None) -> Dict:
    ...
    sessions = []
    timestamps = []
    for session_num in sorted(set(session_nums)):
...
def run_lightmem_official(
    ...
    if dataset_type == "locomo" and bool(extras.get("auto_eval", False)):
        ...
        for top_k in retrieve_top_ks:
            ...
            eval_main(eval_dataset, eval_method, version, eval_embedding_model)
