...
def top_k_output_path(base_output_path: str, top_k: int) -> str:
    ...
    output_name = os.path.basename(base_output_path) or "result_simplified.json"
    ...
    for index in range(1000):
        ...
        for dialog in conversation.get(session_id, []) or []:
            ...
            if "blip_caption" in dialog:
                turns.append(f"[{speaker}]: {text}\n The image Caption: {dialog.get('blip_caption', '')}")
            else:
                turns.append(f"[{speaker}]: {text}")
...
def run_memgas(
    ...
    process_defaults = {
        ...
        "llm_context_window": llm_context_window if llm_context_window is not None else 16384,
    ...
    for ordinal, (dataset_index, sample) in enumerate(selected_samples, start=1):
        ...
        tracker = TokenTracker(output_file=sample_token_file) if track_tokens else None
        if tracker is not None:
            tracker.patch_openai()
        try:
            if tracker is not None:
                sample_stage = tracker.stage(f"sample_{dataset_index}")
                sample_stage.__enter__()
            else:
                sample_stage = None
            try:
                ...
                if sample_stage is not None:
                    sample_stage.__exit__(None, None, None)
        finally:
            if tracker is not None:
                tracker.restore()
                tracker.save_to_json()
    ...
    # Preserve the framework-wide contract: output_path contains the primary
    # result, while per-top-k variants remain available in top_k_* folders.
    primary_top_k = max(retrieve_top_ks)
    write_json(output_path, aggregated_by_top_k[primary_top_k])
    ...
    if auto_eval:
        ...
        for top_k in retrieve_top_ks:
            ...
            eval_main(dataset_name, "memgas", version, embedding_model)
