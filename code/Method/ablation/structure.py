...
class MemTree:
    ...
    def _modify_sync(node_id, current_content, len_children, new_content, api_key, base_url, model):
        ...
        output = client.chat_completion(model, messages=[{"role": "user", "content": input_prompt}], stage="tree_aggregation")
    ...
    async def _modify_async(node_id, current_content, len_children, new_content, client, model):
        ...
        output = await client.chat_completion(model, messages=[{"role": "user", "content": input_prompt}], stage="tree_aggregation")
        return node_id, output
