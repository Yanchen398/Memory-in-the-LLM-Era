...
def get_embedding_model(config):
    ...
    model_name = getattr(config, "embedding_model_name", None) or os.getenv(
        "ABLATION_EMBEDDING_MODEL"
    )
    if not model_name:
        raise ValueError(
            "embedding_model_name is required; set it in the config or "
            "ABLATION_EMBEDDING_MODEL"
        )
    embedding_aliases = {
...
class GlobalConfig:
    def __init__(self, config):
        ...
        if getattr(config, "embedding_base_url", None):
            self.model = None
        else:
