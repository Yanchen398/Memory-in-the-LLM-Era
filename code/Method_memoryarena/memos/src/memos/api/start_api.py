...
DEFAULT_CONFIG = {
    ...
    "top_k": int(os.getenv("MOS_TOP_K", "10")),
    "chat_model": {
        "backend": os.getenv("MOS_CHAT_MODEL_PROVIDER", "openai"),
        "config": {
            "model_name_or_path": os.getenv("MOS_CHAT_MODEL", "Qwen3.5-9B"),
            "api_key": os.getenv("OPENAI_API_KEY", "EMPTY"),
            "temperature": float(os.getenv("MOS_CHAT_TEMPERATURE", "0.0")),
...
def get_mos_instance():
    ...
    if MOS_INSTANCE is None:
        if not DEFAULT_CONFIG["chat_model"]["config"]["api_base"]:
            raise RuntimeError(
                "MemOS API requires OPENAI_API_BASE or OPENAI_BASE_URL to point "
                "to an explicit local endpoint"
            )
