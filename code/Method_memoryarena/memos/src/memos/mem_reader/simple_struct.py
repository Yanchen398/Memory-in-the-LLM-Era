...
class SimpleStructMemReader(BaseMemReader, ABC):
    ...
    def _process_chat_data(self, scene_data_info, info):
        ...
        # 使用带重试机制的方法
        # 重测token更改！！
        # response_json = self._generate_and_parse_with_retry(messages)
    ...
    def get_memory(
        ...
        for scene_data_info in list_scene_data_info:
            res_memory = processing_func(scene_data_info, info)
            memory_list.append(res_memory)
    ...
    def _generate_and_parse_with_retry(self, messages, max_retries=1):
        """
        生成 LLM 响应并解析 JSON,失败时自动重试
        ...
        for attempt in range(max_retries):
            try:
                response_text = self.llm.generate(messages)
                parsed_result = self.parse_json_result(response_text)
                
                # 检查是否解析成功(parse_json_result 失败时返回空字典)
                if parsed_result:
                    return parsed_result
                else:
                    logger.warning(f"Attempt {attempt + 1}/{max_retries}: JSON parsing returned empty result")
            except Exception as e:
                logger.warning(f"Attempt {attempt + 1}/{max_retries}: Error during generation or parsing: {e}")
            
            # 如果不是最后一次尝试,记录重试信息
            if attempt < max_retries - 1:
                logger.info(f"Retrying LLM generation... ({attempt + 2}/{max_retries})")
        
        logger.error(f"Failed to generate and parse valid JSON after {max_retries} attempts")
        return {}
