...
async def locomo_response(llm_client, model_name: str, context: str, question: str) -> str:
    ...
    if str(model_name).lower().startswith("gpt-5.4"):
...
async def process_qa(qa, search_result, oai_client, model_name, semaphore):
    start = time()
...
def save_formatted_results(all_responses, locomo_df, runtime_config):
    ...
    for group_idx in range(num_users):
        ...
        for response_data in group_responses:
            retrieval_latency_ms = response_data.get("search_duration_ms", 0.0)
            if isinstance(retrieval_latency_ms, (int, float)):
                retrieval_latencies_ms.append(float(retrieval_latency_ms))
            else:
    ...
    latency_summary = {
        ...
        "query_count": len(retrieval_latencies_ms),
        "average_retrieval_latency_ms": (
            sum(retrieval_latencies_ms) / len(retrieval_latencies_ms)
            if retrieval_latencies_ms
            else 0.0
        ),
        "min_retrieval_latency_ms": min(retrieval_latencies_ms)
        if retrieval_latencies_ms
        else 0.0,
        "max_retrieval_latency_ms": max(retrieval_latencies_ms)
        if retrieval_latencies_ms
        else 0.0,
    }
    latency_path = os.path.join(runtime_config["result_dir"], "retrieval_latency.json")
    with open(latency_path, "w", encoding="utf-8") as f:
        json.dump(latency_summary, f, indent=2)
    print(
        "Average retrieval latency "
        f"(top_k={runtime_config['top_k']}): "
        f"{latency_summary['average_retrieval_latency_ms']:.2f} ms"
    )
...
async def response(runtime_config):
    ...
    semaphore = asyncio.Semaphore(
        max(1, int(runtime_config.get("response_concurrency", 16)))
    )
