...
def parse_top_ks(value):
    if value is None:
        return copy.deepcopy(DEFAULT_RETRIEVE_TOP_KS)
    ...
    for value in values:
        top_k = int(value)
        if top_k <= 0:
            raise ValueError(f"retrieve top-k must be positive, got {top_k}")
        if top_k not in top_ks:
            top_ks.append(top_k)
    if not top_ks:
        raise ValueError("retrieve_top_ks must contain at least one value")
    return top_ks
...
def build_runtime_config(config=None, config_path=None):
    ...
    if not dataset_path:
        raise ValueError(
            "MemOS requires an explicit dataset_path in the runtime config or CLI."
        )
    ...
    retrieve_top_ks = parse_top_ks(config.get("retrieve_top_ks", config.get("top_k")))
    runtime_config = {
        "version": version,
        "dataset_path": dataset_path,
        "num_workers": config.get("num_workers", 4),
        "top_k": retrieve_top_ks[0],
        ...
        "response_concurrency": config.get("response_concurrency", 16),
        ...
        "clear_graph_db": config.get("clear_graph_db", False),
        "clear_existing_index": config.get("clear_existing_index", False),
        "skip_ingestion": config.get("skip_ingestion", False),
        "clear_previous_results": config.get("clear_previous_results", False),
        "auto_eval": config.get("auto_eval", False),
        "eval_embedding_model": config.get(
            "eval_embedding_model", "/path/to/local/Qwen3-Embedding-0.6B"
        ),
        "simplify_before_eval": config.get("simplify_before_eval", True),
        "simplify_temperature": config.get("simplify_temperature", 0.0),
        "simplify_max_tokens": config.get("simplify_max_tokens", 150),
    }
...
def build_retrieval_runtime_config(runtime_config, top_k):
    config = copy.deepcopy(runtime_config)
    config["top_k"] = int(top_k)

    if len(runtime_config["retrieve_top_ks"]) > 1:
        ...
        config.update(
            {
                ...
                "search_results_path": os.path.join(
                    result_dir, "memos_locomo_search_results.json"
                ),
                "response_results_path": os.path.join(
                    result_dir, "memos_locomo_responses.json"
                ),
                "formatted_results_path": os.path.join(result_dir, "result.json"),
                "simplified_results_path": os.path.join(
                    result_dir, "result_simplified.json"
                ),
            }
        )
    else:
        config["simplified_results_path"] = os.path.join(
            config["result_dir"], "result_simplified.json"
        )
    return config
