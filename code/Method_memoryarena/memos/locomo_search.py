...
def get_client(user_id: str, runtime_config, top_k: int = 10):
...
def process_user(group_idx, locomo_df, runtime_config, top_k=10, num_workers=1):
...
def search(runtime_config):
    ...
    for idx in range(num_users):
        try:
            ...
            message = f"User {idx} generated an exception: {e}"
            print(message)
            errors.append(message)

    if errors:
        raise RuntimeError(
            f"Memos search failed for {len(errors)} user(s): " + "; ".join(errors)
        )
