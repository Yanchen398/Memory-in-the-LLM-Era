...
try:
    from .main import build_qwen_openai_client
except ImportError:
    from Method.zep.main import build_qwen_openai_client
...
async def generate_answer(
    ...
    try:
        ...
        response = await client.chat.completions.create(
            ...
            max_tokens=200,
            temperature=0,
            extra_body=QWEN_DISABLE_THINKING_EXTRA_BODY,
        )
...
async def main():
    parser = argparse.ArgumentParser(description="Materialize Zep benchmark answers.")
    parser.add_argument("--input-file", required=True)
    parser.add_argument("--output-file")
    parser.add_argument("--model", default=DEFAULT_ANSWER_MODEL)
    parser.add_argument("--api-key", default=DEFAULT_ANSWER_API_KEY)
    parser.add_argument("--base-url", default=DEFAULT_ANSWER_BASE_URL)
    args = parser.parse_args()
    input_file = args.input_file
    output_file = args.output_file or input_file.replace("result_raw.json", "result.json")
    ...
    try:
        ...
        for i, sample in enumerate(data):
            try:
                ...
                updated_sample = await process_sample(
                    sample,
                    model=args.model,
