...
class MemoryGraph:
    ...
    def add(self, data, filters):
        ...
        print("entity_type_map", entity_type_map, flush=True)
        to_be_added = self._establish_nodes_relations_from_data(data, filters, entity_type_map)
        print("to_be_added", to_be_added, flush=True)
        search_output = self._search_graph_db(node_list=list(entity_type_map.keys()), filters=filters)
        print("search_output", search_output, flush=True)
        to_be_deleted = self._get_delete_entities_from_search_output(search_output, data, filters)
        print("to_be_deleted", to_be_deleted, flush=True)
    ...
    def _retrieve_nodes_from_data(self, data, filters):
        ...
        print(f"Retrieving nodes from data: {data}")
        ...
        entity_type_map = {}
        print(f"search_results nodes_from_data222: {search_results}", flush=True)
        ...
        print(f"Extracted entity_type_map: {entity_type_map}", flush=True)
    ...
    def _establish_nodes_relations_from_data(self, data, filters, entity_type_map):
        ...
        print("Before generate_response_establish", flush=True)
        extracted_entities = self.llm.generate_response(
            messages=messages,
            tools=_tools,
        )
        print("After generate_response_establish", flush=True)
        entities = []
        ...
        print(f"Extracted entities2: {entities}", flush=True)
