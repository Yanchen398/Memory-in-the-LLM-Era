...
def get_embedding_model(config):
    ...
    model_name = getattr(config, "embedding_model_name", "/path/to/local/all-MiniLM-L6-v2")
    embedding_device = getattr(config, "embedding_device", os.environ.get("MEMTREE_EMBEDDING_DEVICE", "cpu"))
    embedding_aliases = {
        "minilm": "/path/to/local/all-MiniLM-L6-v2",
        "all-minilm-l6-v2": "/path/to/local/all-MiniLM-L6-v2",
        "all-MiniLM-L6-v2": "/path/to/local/all-MiniLM-L6-v2",
        "bge-m3": "/path/to/local/bge-m3",
        "models--BAAI--bge-m3": "/path/to/local/bge-m3",
    }
    resolved_model_name = embedding_aliases.get(model_name, model_name)
    model = SentenceTransformer(resolved_model_name, device=embedding_device)
    print(f"Loaded memtree embedding model {resolved_model_name} on {embedding_device}")
...
def resolve_memtree_config_dict(config, config_path=None):
    ...
    if config_path:
        ...
        base_dir = repo_root if os.path.basename(config_dir) == "Config" else config_dir
    else:
        base_dir = os.getcwd()
