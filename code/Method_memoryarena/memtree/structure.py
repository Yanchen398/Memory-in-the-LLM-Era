    ...
    def add_node(self, content: str, parent_id: Optional[int] = None) -> int:
        ...
        while is_continue_traversal:
            ...
            try:
                ...
                raise RuntimeError(
                    "MemTree cosine calculation failed: "
                    f"query_shape={np.shape(ev)}, child_shape={np.shape(node_embs_in_next_layer)}, "
    ...
    def modify_nodes(self, new_content: str, node_ids: List[int]):
        if node_ids:
            ...
            with tqdm(total=total_tasks, desc="Processing updation of parent nodes traversed along the path...") as pbar:
                parallel_nums = int(getattr(globalconfig, 'llm_parallel_nums', 1) or 1)
                parallel_nums = max(1, min(parallel_nums, total_tasks))
                if parallel_nums == 1:
                    for task in tasks:
                        node_id, output = self._modify_shared_mem(task)
                    ...
                    with ThreadPoolExecutor(max_workers=parallel_nums) as executor:
                        futures = [executor.submit(self._modify_shared_mem, task) for task in tasks]
