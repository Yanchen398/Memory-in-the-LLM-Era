        ...
        def _parse_json(text: str) -> Dict[str, Any]:
            ...
            if isinstance(parsed_value, list):
                if not all(
                    ...
                    and isinstance(entry.get("attribute"), str)
                    and isinstance(entry.get("has_attribute"), bool)
                    for entry in parsed_value
                ):
                    raise ValueError("Top-level judgment list has invalid entries.")
                return {"matches": parsed_value}
            if isinstance(parsed_value, dict):
                return parsed_value
            start, end = cleaned.find("{"), cleaned.rfind("}")
            if start < 0 or end < 0 or end <= start:
                raise ValueError("No JSON object in response.")
            raise ValueError("Response JSON is neither an object nor an attribute-judgment list.")
...
def display_path(path: Path) -> Path:
    try:
        return path.relative_to(MEMORYARENA_ROOT)
    except ValueError:
        return path
...
def main() -> None:
    ...
    if processed:
        ...
        for s in processed:
            rel = display_path(Path(s["run_dir"]))
            print(
                f"  {rel}\n"
