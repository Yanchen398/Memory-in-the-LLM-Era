...
from ..dataset_hygiene import natural_session_keys, pair_session_turns
...
def build_session_pairs(conversation_data: Dict, session_key: str, speaker_a: str, speaker_b: str) -> List[Dict]:
    return pair_session_turns(conversation_data, session_key, speaker_a, speaker_b)
...
def convert_sample_memory(
    ...
    for session_key in session_keys:
        session_time = conversation_data.get(f"{session_key}_date_time") or "timestamp unavailable"
        session_pairs = build_session_pairs(conversation_data, session_key, speaker_a, speaker_b)
        if not session_pairs:
            continue

        # Date strings are not unique in LOCOMO variants.  Include the source
        # session key so a later session can never overwrite an earlier one.
        session_label = f"{session_key} @ {session_time}"
        history[session_label] = session_pairs
        ...
        session_summaries[session_label] = summary
        session_personality[session_label] = personality
