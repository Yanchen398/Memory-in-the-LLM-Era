from .utils import gpt_summarize, generate_id, get_timestamp, gpt_update_profile, gpt_generate_multi_summary, get_llm_model

class DynamicUpdate:
    def __init__(self, short_term_memory, mid_term_memory, long_term_memory, topic_similarity_threshold=0.8, client=None, llm_model=None, fast_index=False):
...
Continuous?""".format(
    ...
    def _generate_meta_info(self, last_page_meta, current_page):
        """
        Generate new meta-info from the previous page's meta-info and the current page content.
        :param last_page_meta: Meta-info from the previous page.
        :param current_page: Dialog content on the current page.
        :return: The updated meta-info.
        """
    ...
    def bulk_evict_and_update_mid_term(self, force=False):
        evicted = []
        # 1. Remove a full batch from short-term memory.
        ...
        # 2. Create base page structures and then process continuity.
        pages = []
        for qa in evicted:
            page = {
            ...
            # Check conversational continuity.
        ...
        # 3. Concatenate all user inputs for topic analysis.
        input_text = "\n".join([f"User: {page.get('user_input','')}\n" for page in pages])
        print("Dynamic update: calling GPT to generate multi-topic summaries...")
        multi_summary = gpt_generate_multi_summary(input_text, self.client)
        
        # 4. Group by topic and insert into mid-term memory.
        for summary_dict in multi_summary.get("summaries", []):
            if not isinstance(summary_dict, dict):
                print(f"Dynamic update: skipping malformed summary item: {summary_dict!r}")
                continue
            ...
            if self.fast_index:
                for page in pages:
                    ...
                    page["page_keywords"] = sub_key_words
            print(f"Dynamic update: processing subtopic [{summary_dict.get('theme','')}] and inserting it into mid-term memory...")
            self.mid_term_memory.insert_pages_into_session(
                sub_summary, 
                sub_key_words, 
                pages,  # Pass in the fully processed pages.
                self.topic_similarity_threshold
            )

    def update_long_term(self, user_id, new_profile_data, knowledge_text):
        print("Dynamic update: updating the user profile and private knowledge in long-term memory...")
