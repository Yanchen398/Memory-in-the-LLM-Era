...
class TokenTracker:
    ...
    def patch_llm_api(self, 
        ...
        def _token_counting_wrapper(*args, **kwargs):
            ...
            stack = getattr(self._context_stack, 'value', None)
            with self._lock:
                if stack:
                    current_stage = stack[-1]
                    ...
                    # Worker threads do not inherit the parent's stage stack.
    ...
    def stage(self, name: str):
        ...
        if not stack:
            stack = [self.root]
            self._context_stack.value = stack
        parent_stage = stack[-1]
