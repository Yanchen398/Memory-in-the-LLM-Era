...
def build_page_embedding_text(page):
    return f"{page.get('user_input','')}{page.get('timestamp','')}{page.get('agent_response','')}"

class MidTermMemory:
    def __init__(self, max_capacity=7, file_path="mid_term.json", client=None):
    ...
    def evict_lfu(self):
        ...
        print(f"Mid-term memory: LFU evicted session segment {lfu_sid}.")
    ...
    def add_session(self, summary, details):
        ...
        for page in details:
            ...
            full_text = build_page_embedding_text(page)
        ...
        print(f"Mid-term memory: added session segment {session_id} with initial heat {session_obj['H_segment']:.2f}.")
    ...
    def insert_pages_into_session(self, summary, keyworks, pages, similarity_threshold=0.6, alpha=1.0):
        ...
        if best_sim >= 0 and best_sid is not None:
            print(f"Mid-term memory: trying to merge into session segment {best_sid} (summary similarity {best_sim:.2f}).")
            ...
            if overall_score >= similarity_threshold:
                print(f"Mid-term memory: overall score {overall_score:.2f} meets the merge threshold; appending pages.")
                for p in pages:
                    if "page_id" not in p:
                        p["page_id"] = generate_id("page")
                    full_text = build_page_embedding_text(p)
                ...
                print("Mid-term memory: overall score is below threshold; creating a new session segment.")
                self.add_session(summary, pages)
        else:
            print("Mid-term memory: no similar session segment found; creating a new one.")
            self.add_session(summary, pages)
    ...
    def search_sessions_by_summary(self, query, client, segment_threshold=0.8, page_threshold=0.7, top_k=5, tau=3600, gamma=0.5, alpha=1.0, update_stats=True, use_llm_keywords=True):
        ...
        if use_llm_keywords:
            keyword_client = client or self.client
            query_keywords = llm_extract_keywords(query, keyword_client)
        else:
            query_keywords = set()
        ...
        for dist, idx in zip(distances[0], indices[0]):
            ...
            if overall >= segment_threshold:
                matched_pages = []
                for page in session["details"]:
                    stored_embedding = page.get("page_embedding")
                    if stored_embedding is not None:
                        pvec = np.array(stored_embedding, dtype=np.float32)
                    else:
                        full_text = build_page_embedding_text(page)
                ...
                if matched_pages:
                    if update_stats:
                        ...
                        session["H_segment"] = compute_segment_heat(session)
                    print(f"Mid-term memory: session segment {sid} matched {len(matched_pages)} QA pairs.")
                    results.append({
                        "session": session,
    ...
    def save(self):
        ...
        print("Mid-term memory: saved successfully.")
    ...
    def load(self):
        try:
            ...
            self.rebuild_heap()
            print("Mid-term memory: loaded successfully.")
        except Exception:
            self.sessions = {}
            self.access_frequency = defaultdict(int)
            self.heap = []
            print("Mid-term memory: no historical data found.")
