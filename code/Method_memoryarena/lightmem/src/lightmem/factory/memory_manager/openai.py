...
class OpenaiManager:
    def __init__(self, config: BaseMemoryManagerConfig):
        ...
        if os.environ.get("OPENROUTER_API_KEY"):
            raise RuntimeError("External OpenRouter access is disabled for benchmark runs")

        api_key = self.config.api_key or os.getenv("OPENAI_API_KEY") or "EMPTY"
        base_url = (
        ...
        if not base_url:
            raise RuntimeError(
                "LightMem requires an explicit local OpenAI-compatible base URL"
            )
